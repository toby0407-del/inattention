...
flowchart LR
  ...
  Ctx --> S8["關卡難度：`spotTimeLimitSec`、`spotMaxMistakes`、`memoryShowSeconds`、`memoryPlayLimitSec`"]
```
...
| `src/app/pages/QuestMap.tsx` | 星圖 UI、章節切換、圖鑑、問號「寓意+典故」彈窗 |
| `src/app/components/ConstellationGlobe.tsx` | 圖鑑內地球＋十二星座完成配色 |
| `src/app/pages/Gameplay.tsx` | 關卡遊玩 |
| `src/app/components/PhotoHuntSpotDiff.tsx` | 找不同：時間上限與試錯上限、錯誤揭示 |
| `src/app/data/openDataGameSets.ts` | 開源 OpenMoji 題庫（順序排列 / 記憶配對） |
