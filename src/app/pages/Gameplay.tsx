...
import { pickMemoryTheme, pickOrderStory, type VisualCard } from '../data/openDataGameSets';
...
// ===================== ORDER SORT =====================
type OrderCardData = Required<Pick<VisualCard, 'id' | 'caption' | 'emoji' | 'order'>> & { bg?: string };

const EMOJI_FONT = '"Segoe UI Emoji","Apple Color Emoji","Noto Color Emoji","Twemoji Mozilla",sans-serif';

function VisualToken({ card, size = 40 }: { card: { caption: string; emoji?: string }; size?: number }) {
  return (
    <span style={{ fontSize: size * 0.85, lineHeight: 1, fontFamily: EMOJI_FONT }}>
      {card.emoji ?? '🧩'}
    </span>
  );
}
...
function OrderDraggable({ card }: { card: OrderCardData }) {
  ...
  return (
    <div
      ref={drag as any}
      className="rounded-2xl flex flex-col items-center justify-center cursor-grab active:cursor-grabbing select-none"
      style={{
        background: card.bg ?? 'rgba(255,255,255,0.14)',
        opacity: isDragging ? 0.45 : 1,
        width: 72,
        minHeight: 82,
        border: '2px solid rgba(255,255,255,0.55)',
        boxShadow: isDragging ? 'none' : '0 6px 18px rgba(0,0,0,0.28), inset 0 1px 0 rgba(255,255,255,0.6)',
        touchAction: 'none',
        gap: 4,
        padding: '8px 4px 6px',
      }}
    >
      <VisualToken card={card} size={36} />
      <span style={{ fontSize: 10, fontWeight: 800, color: '#334155', marginTop: 2, textAlign: 'center', lineHeight: 1.2 }}>
        {card.caption}
      </span>
    </div>
  );
...
function OrderDropSlot({
  ...
  const card = placedId !== null ? cards.find(c => c.id === placedId) : null;

  return (
    <div
      ref={drop as any}
      className="rounded-2xl flex flex-col items-center justify-center transition-all relative px-1 py-2"
      style={{
        border: highlightWrong
          ? '3px solid #ff6b6b'
          : isOver
        ...
        minWidth: 76,
        minHeight: 90,
        boxShadow: card ? '0 4px 14px rgba(0,0,0,0.18)' : undefined,
    ...
    >
      {card ? (
        ...
        <>
          <span style={{ fontSize: 18, color: 'rgba(255,255,255,0.25)', fontWeight: 900 }}>
            {slotIndex + 1}
          </span>
          <span style={{ fontSize: 9, fontWeight: 700, color: 'rgba(255,255,255,0.2)', marginTop: 2 }}>
            順位
          </span>
        </>
      )}
...
function OrderSortGame({ onComplete, zodiac, levelId }: { onComplete: () => void; zodiac?: ZodiacInfo; levelId: number }) {
  const orderStory = useMemo(() => pickOrderStory(levelId), [levelId]);
  const cards = orderStory.cards as OrderCardData[];
  const [slots, setSlots] = useState<(number | null)[]>(() => Array(cards.length).fill(null));
  const [tray, setTray] = useState<number[]>(() => shuffleArray(cards.map(c => c.id)));
  const [checkHint, setCheckHint] = useState<string | null>(null);
  const [slotWrong, setSlotWrong] = useState<boolean[]>(() => Array(cards.length).fill(false));

  useEffect(() => {
    setSlots(Array(cards.length).fill(null));
    setTray(shuffleArray(cards.map(c => c.id)));
    setSlotWrong(Array(cards.length).fill(false));
    setCheckHint(null);
  }, [cards]);
  ...
  function handleCheck() {
    ...
    const wrong = slots.map((id, i) => cards.find(c => c.id === id)!.order !== i);
    if (wrong.some(Boolean)) {
      setSlotWrong(wrong);
      setCheckHint(`順序不太對喔！請依照「${orderStory.title}」的事件先後重新排列。`);
      return;
    }
  ...
  return (
    <DndProvider backend={TouchBackend} options={DND_OPTIONS}>
      <div data-dnd-root className="flex flex-col h-full px-4 pb-3 overflow-y-auto overscroll-contain">
        {/* 主題橫幅 */}
        <div
          className="mt-2 mb-1 mx-auto px-5 py-2 rounded-2xl text-white text-center"
          style={{ background: orderStory.color, fontWeight: 900, fontSize: 16, boxShadow: '0 4px 14px rgba(0,0,0,0.25)', maxWidth: 360 }}
        >
          {orderStory.title}
        </div>
        <div className="text-center text-white/60 pb-1" style={{ fontWeight: 700, fontSize: 12 }}>
          把下面圖卡拖進上面格子，排出先後順序
        </div>
        ...
        <OrderTrayStrip onReturnCard={returnCardToTray}>
          {tray.map(id => (
            <OrderDraggable key={id} card={cards.find(c => c.id === id)!} />
          ))}
          {tray.length === 0 && (
...
// ===================== MEMORY DROP =====================
type MemoryItem = Omit<VisualCard, 'order'>;
...
function MemoryDraggable({ item, disabled }: { item: MemoryItem; disabled?: boolean }) {
  ...
  return (
    <div
      ref={drag as any}
      className="rounded-2xl flex flex-col items-center justify-center select-none gap-1"
      style={{
        background: item.bg ?? 'rgba(255,255,255,0.18)',
        opacity: isDragging ? 0.45 : disabled ? 0.75 : 1,
        width: 68,
        minHeight: 78,
        padding: '8px 4px 6px',
        border: '2px solid rgba(255,255,255,0.55)',
        boxShadow: isDragging || disabled ? 'none' : '0 5px 14px rgba(0,0,0,0.22), inset 0 1px 0 rgba(255,255,255,0.55)',
        touchAction: 'none',
        cursor: disabled ? 'default' : 'grab',
      }}
    >
      <VisualToken card={item} size={36} />
      <span style={{ fontSize: 10, fontWeight: 800, color: '#334155', textAlign: 'center', lineHeight: 1.2 }}>
        {item.caption}
      </span>
    </div>
  );
...
function MemoryDropSlot({
  ...
  const showPeek = phase === 'memorize' && revealedItem;
    ...
    >
      ...
      {showPeek && (
        <motion.div
          ...
          style={{
            background: revealedItem.bg ?? 'rgba(255,255,255,0.18)',
        ...
        >
          <VisualToken card={revealedItem} size={38} />
          <span style={{ fontSize: 10, fontWeight: 800, color: '#334155', lineHeight: 1.2 }}>
            {revealedItem.caption}
          </span>
        </motion.div>
      )}
...
function MemoryMatchGame({
  ...
  const theme = useMemo(() => pickMemoryTheme(levelId), [levelId]);
  ...
  /** 解題剩餘秒數 */
  const [secLeft, setSecLeft] = useState(Math.floor(playLimitSec));
  ...
  useEffect(() => {
    ...
    }, Math.round(showSeconds * 1000));
    return () => window.clearTimeout(t);
  }, [themeItems, showSeconds]);
    ...
    <DndProvider backend={TouchBackend} options={DND_OPTIONS}>
      <div data-dnd-root className="flex flex-col h-full px-4 pb-3 overflow-y-auto overscroll-contain">
        ...
        <div className="flex items-center justify-between gap-2 pt-2 pb-1">
          <div
            className="px-4 py-1.5 rounded-2xl text-white"
            style={{ background: theme.color, fontWeight: 900, fontSize: 14, boxShadow: '0 3px 10px rgba(0,0,0,0.22)' }}
          >
            {theme.label}
          </div>
        ...
        {phase === 'memorize' && (
          ...
          >
            提示：約 {showSeconds.toFixed(1)} 秒後進入作答
          </motion.div>
        )}
          ...
          <div className="flex justify-center gap-3 mb-2 flex-wrap">
            <button
              ...
              onClick={() => {
                ...
                setCheckHint(null);
                setSecLeft(Math.floor(playLimitSec));
              }}
...
export default function Gameplay() {
  ...
  function handleExit() {
    navigate('/child/lobby', { state: { fromLevel: level } });
  }
      ...
      <div className="flex-1 overflow-hidden" style={{ pointerEvents: paused ? 'none' : 'auto' }}>
        {mode === 'spot' ? (
          <PhotoHuntSpotDiff
            ...
            levelId={levelMeta.id ?? level}
            seconds={spotTimeLimitSec}
            maxMistakes={spotMaxMistakes}
          />
        ) : mode === 'jigsaw' ? (
          <JigsawGame onComplete={handleComplete} zodiac={levelMeta.zodiac} levelId={levelMeta.id ?? level} />
        ) : mode === 'order' ? (
          <OrderSortGame onComplete={handleComplete} zodiac={levelMeta.zodiac} levelId={levelMeta.id ?? level} />
        ) : (
          <MemoryMatchGame
            onComplete={handleComplete}
            zodiac={levelMeta.zodiac}
            levelId={levelMeta.id ?? level}
            showSeconds={memoryShowSeconds}
            playLimitSec={memoryPlayLimitSec}
          />
        )}
      </div>
