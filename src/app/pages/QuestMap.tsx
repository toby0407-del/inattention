...
import { ChevronLeft, Settings, MessageCircle, Send, X } from 'lucide-react';
...
// ── Child Chat Overlay ────────────────────────────────────────────────────────
function ChildChatOverlay({ onClose }: { onClose: () => void }) {
  const { messages, send, markAllRead } = useMessages();
  const { selectedChild } = useApp();
  const [input, setInput] = useState('');
  const [sending, setSending] = useState(false);
  const bottomRef = useRef<HTMLDivElement>(null);

  useEffect(() => { markAllRead('child'); }, [messages.length]);
  ...
  return (
    <motion.div
      ...
      style={{ background: 'rgba(2,6,23,0.92)', backdropFilter: 'blur(14px)' }}
    ...
    >
      ...
      >
        ...
        <div className="flex-1">
          <div className="text-white" style={{ fontWeight: 800, fontSize: 16 }}>林醫師</div>
          <div className="text-emerald-400" style={{ fontSize: 11, fontWeight: 600 }}>訊息頻道</div>
        </div>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          ...
          {messages.map(msg => {
            ...
            return (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
              >
                {!isMe && <span className="text-xl mr-2 self-end mb-1">👨‍⚕️</span>}
                <div className="max-w-[75%]">
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: isMe ? '0 4px 14px rgba(124,58,237,0.35)' : undefined,
                    }}
                  >
                    {msg.text}
                  </div>
                  <div
                    className={`mt-1 ${isMe ? 'text-right' : 'text-left'}`}
                ...
                {isMe && <span className="text-xl ml-2 self-end mb-1">{selectedChild.avatar}</span>}
              </motion.div>
            );
          })}
        </AnimatePresence>
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-40 text-white/30">
            ...
            <p className="mt-2" style={{ fontSize: 13, fontWeight: 600 }}>醫師還沒有傳訊息喔</p>
          </div>
        )}
        <div ref={bottomRef} />
      </div>
      ...
      {/* Input */}
      <div
        className="px-4 py-4 flex items-end gap-3 shrink-0 border-t"
      ...
      >
        <input
          ...
          onKeyDown={handleKey}
          placeholder="傳訊息給林醫師…"
          className="flex-1 rounded-2xl px-4 py-3 text-white focus:outline-none"
          style={{
        ...
        <button
          ...
          style={{
            width: 46,
            height: 46,
            background: input.trim() ? 'linear-gradient(135deg,#7c3aed,#6366f1)' : 'rgba(255,255,255,0.12)',
            flexShrink: 0,
            border: '1.5px solid rgba(255,255,255,0.2)',
          }}
        >
...
function centerPercentCoords(points: { x: number; y: number }[]): { x: number; y: number }[] {
  ...
  return points.map(p => ({
    x: clamp((p.x - cx) * scale + 50),
    y: clamp((p.y - cy) * scale + 50),
  }));
}
...
export default function QuestMap() {
  ...
  /** 優先以「剛離開的關卡」決定顯示章節，再 fallback 到最新進度 */
  const [chapterIndex, setChapterIndex] = useState<number>(() => {
    const fromLevel = (location.state as { fromLevel?: number } | null)?.fromLevel;
    // 若剛玩的是舊關卡（重玩），以 currentLevel 為準；否則用 fromLevel
    const effectiveLevel = Math.max(fromLevel ?? 0, currentLevel);
    const ch = getChapterForLevel(effectiveLevel) ?? getChapterForLevel(currentLevel);
    if (!ch) return Math.max(0, CHAPTERS.length - 1);
  ...
  /** currentLevel 更新時（完成新關卡）同步切到對應章節 */
  useEffect(() => {
  ...
  const bg = useMemo(() => {
    ...
    if (chapter.mapTheme === 'space' || chapter.mapTheme === 'finale') {
      return '#020617';
    }
  ...
  const constellationMyth = useMemo(() => {
    const direct = chapter.levels[0]?.zodiac?.myth?.trim();
    if (direct) return direct;
    const fromLevel = chapter.levels.map(l => l.zodiac?.myth?.trim()).find(Boolean);
    return fromLevel ?? '';
  }, [chapter]);
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute top-0 left-0 right-0 z-[45] flex items-start justify-between px-4 pt-3 pb-3 sm:px-6 sm:pt-4 gap-3 pointer-events-none"
        style={{ background: 'linear-gradient(to bottom, rgba(0,0,0,0.55) 0%, rgba(0,0,0,0.2) 70%, transparent 100%)' }}
      >
        <motion.button
          type="button"
          onClick={() => navigate('/')}
          className="flex items-center gap-2 pl-1.5 pr-3 sm:pr-4 py-2 rounded-full text-white shadow-lg active:scale-[0.97] transition-transform border-2 shrink-0 pointer-events-auto"
          style={{
        ...
        <div className="flex items-start gap-2 shrink-0 pointer-events-auto">
        {/* Message button with unread badge */}
        <motion.button
          type="button"
          onClick={() => setShowChat(true)}
          className="relative flex items-center gap-1.5 px-3 py-2 rounded-2xl text-white shadow-md active:scale-[0.97] transition-transform border"
          style={{
          ...
          aria-label="開啟訊息"
        >
          <MessageCircle className="w-5 h-5 text-white shrink-0" strokeWidth={2.2} aria-hidden />
          <span className="hidden sm:inline" style={{ fontWeight: 900, fontSize: '13px' }}>訊息</span>
          {unreadCount > 0 && (
            <span
              className="absolute -top-1.5 -right-1.5 rounded-full flex items-center justify-center text-white"
              style={{ fontSize: 9, fontWeight: 800, background: '#f43f5e', minWidth: 16, height: 16, padding: '0 3px' }}
            >
              {unreadCount}
            </span>
          )}
        ...
        {chapterTipMode && (
          ...
          >
            ...
            >
              ...
              >
                <div className="text-sky-100/80 mb-1.5" style={{ fontWeight: 800, fontSize: '12px' }}>
                  典故小故事
                </div>
                <p className="text-sky-50" style={{ fontWeight: 700, fontSize: '15px', lineHeight: 1.6 }}>
                  {constellationMyth || '此星座典故整理中，後續會補上更完整版本。'}
                </p>
              </div>
              <p className="text-white/55" style={{ fontWeight: 600, fontSize: '12px', lineHeight: 1.5 }}>
                名稱與日期在畫面上方區塊；請點圓形外框 <strong className="text-white">？</strong> 開啟本視窗看寓意與典故。
              </p>
              <button
                type="button"
      ...
      {/* Child chat overlay */}
      <AnimatePresence>
        {showChat && <ChildChatOverlay onClose={() => setShowChat(false)} />}
      </AnimatePresence>
          ...
          >
            ...
            <motion.div
              ...
              style={{
                ...
                boxShadow: stickerBookFullscreen ? 'none' : '0 -8px 40px rgba(0,0,0,0.35)',
            ...
            >
              <div className={`flex items-start justify-between gap-3 shrink-0 ${stickerBookFullscreen ? 'mb-2' : 'mb-4'}`}>
                <div className="min-w-0" style={{ fontWeight: 900, fontSize: '17px', color: '#e2e8f0', lineHeight: 1.35 }}>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    className="p-2.5 rounded-2xl text-slate-300 hover:bg-white/10 text-2xl leading-none"
                    aria-label="關閉圖鑑"
                  >
                    ✕
                  </button>
              ...
              {!stickerBookFullscreen && (
                <>
              <div className="mb-3" style={{ fontWeight: 900, fontSize: '15px', color: '#e2e8f0' }}>
                詳細紀錄
              </div>
              ...
              {CHAPTERS.map(ch => {
                ...
                return (
                  <div key={ch.id} className="mb-6 pb-5 border-b border-white/10 last:border-0">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div style={{ fontWeight: 900, fontSize: '15px', color: '#e2e8f0' }}>
                          {ch.title}
                        </div>
                      ...
                      {ch.levels.map(lvl => {
                        ...
                        return (
                          <div
                            ...
                            style={{
                              background: unlocked ? 'linear-gradient(135deg, #fcd34d, #fb923c)' : 'rgba(255,255,255,0.06)',
                              border: unlocked ? '2px solid rgba(251,146,60,0.4)' : '2px dashed rgba(255,255,255,0.15)',
                            }}
                          >
                            <span style={{ opacity: unlocked ? 1 : 0.25 }}>{unlocked ? lvl.collectible.emoji : '🔒'}</span>
                            <span style={{ fontSize: '9px', fontWeight: 900, color: unlocked ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.3)', marginTop: 4, textAlign: 'center', lineHeight: 1.2 }}>
                              主星{starN}
                            </span>
                          </div>
                        );
                    ...
                    <div
                      ...
                      style={{
                        background: assembled ? 'linear-gradient(135deg, #34d399, #38bdf8)' : 'rgba(255,255,255,0.07)',
                        border: `1.5px solid ${assembled ? 'rgba(16,185,129,0.5)' : 'rgba(255,255,255,0.15)'}`,
                        boxShadow: assembled ? '0 8px 24px rgba(16,185,129,0.25)' : 'none',
                      }}
                    >
                      <div
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div
                          ...
                          style={{
                            fontWeight: 800,
                            fontSize: '14px',
                            color: assembled ? 'rgba(255,251,235,0.95)' : '#94a3b8',
                          }}
                        >
                      ...
                      <div
                        ...
                        style={{
                          borderColor: assembled ? 'rgba(255,255,255,0.35)' : 'rgba(255,255,255,0.18)',
                          minHeight: '3rem',
                        }}
                      >
                        <div style={{ fontWeight: 700, fontSize: '11px', color: assembled ? 'rgba(255,255,255,0.8)' : 'rgba(148,163,184,0.9)' }}>
                          太陽星座約
                        </div>
                        <div
                          className="whitespace-nowrap tabular-nums"
                          style={{ fontWeight: 900, fontSize: '15px', color: assembled ? '#fff' : '#cbd5e1' }}
                        >
              ...
              <div className="mt-2 text-center" style={{ fontWeight: 700, fontSize: '13px', color: 'rgba(148,163,184,0.85)' }}>
