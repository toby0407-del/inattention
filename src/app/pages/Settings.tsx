    ...
    >
      ...
      >
        <div className="flex gap-4 mb-5">
          ...
          <div className="min-w-0">
            <div style={{ fontWeight: 700, fontSize: '15px', color: '#1e293b' }}>遊戲難度微調（醫生端）</div>
            <div style={{ fontWeight: 500, fontSize: '13px', color: '#64748b', marginTop: '2px' }}>
              可直接調整秒數、試錯次數與記憶階段時長，針對不同孩子做更精準介入。
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5">
          <div>
            <div className="flex items-center justify-between">
              <div style={{ fontWeight: 800, fontSize: '13px', color: '#334155' }}>找不同時間上限</div>
              <div className="px-3 py-1 rounded-xl text-white" style={{ background: 'linear-gradient(135deg, #0ea5e9, #6366f1)', fontWeight: 900, fontSize: '13px' }}>
                {spotTimeLimitSec}s
              </div>
            </div>
            <input
              ...
              max={120}
              step={5}
              value={spotTimeLimitSec}
              onChange={e => setSpotTimeLimitSec(Number(e.target.value))}
              className="w-full h-2 rounded-full appearance-none cursor-pointer mt-3"
              style={{ background: `linear-gradient(to right, #0ea5e9 ${(spotTimeLimitSec - 20) / 100 * 100}%, #e2e8f0 0%)`, accentColor: '#0ea5e9' }}
            />
          </div>

          <div>
            <div className="flex items-center justify-between">
              <div style={{ fontWeight: 800, fontSize: '13px', color: '#334155' }}>找不同試錯上限</div>
              <div className="px-3 py-1 rounded-xl text-white" style={{ background: 'linear-gradient(135deg, #ef4444, #f97316)', fontWeight: 900, fontSize: '13px' }}>
                {spotMaxMistakes} 次
              </div>
            </div>
            <input
              ...
              max={12}
              step={1}
              value={spotMaxMistakes}
              onChange={e => setSpotMaxMistakes(Number(e.target.value))}
              className="w-full h-2 rounded-full appearance-none cursor-pointer mt-3"
              style={{ background: `linear-gradient(to right, #ef4444 ${(spotMaxMistakes - 2) / 10 * 100}%, #e2e8f0 0%)`, accentColor: '#ef4444' }}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <div className="flex items-center justify-between">
                <div style={{ fontWeight: 800, fontSize: '13px', color: '#334155' }}>記憶展示秒數</div>
                <div className="px-3 py-1 rounded-xl text-white" style={{ background: 'linear-gradient(135deg, #14b8a6, #0ea5e9)', fontWeight: 900, fontSize: '13px' }}>
                  {memoryShowSeconds.toFixed(1)}s
                </div>
              </div>
              <input
                type="range"
                ...
                max={10}
                step={0.5}
                value={memoryShowSeconds}
                onChange={e => setMemoryShowSeconds(Number(e.target.value))}
                className="w-full h-2 rounded-full appearance-none cursor-pointer mt-3"
                style={{ background: `linear-gradient(to right, #14b8a6 ${(memoryShowSeconds - 3) / 7 * 100}%, #e2e8f0 0%)`, accentColor: '#14b8a6' }}
              />
            </div>
            <div>
              <div className="flex items-center justify-between">
                <div style={{ fontWeight: 800, fontSize: '13px', color: '#334155' }}>記憶作答秒數</div>
                <div className="px-3 py-1 rounded-xl text-white" style={{ background: 'linear-gradient(135deg, #8b5cf6, #6366f1)', fontWeight: 900, fontSize: '13px' }}>
                  {memoryPlayLimitSec}s
                </div>
              </div>
              <input
                type="range"
                ...
                max={90}
                step={5}
                value={memoryPlayLimitSec}
                onChange={e => setMemoryPlayLimitSec(Number(e.target.value))}
                className="w-full h-2 rounded-full appearance-none cursor-pointer mt-3"
                style={{ background: `linear-gradient(to right, #8b5cf6 ${(memoryPlayLimitSec - 15) / 75 * 100}%, #e2e8f0 0%)`, accentColor: '#8b5cf6' }}
              />
            </div>
          </div>
        </div>
        ...
        >
          ...
          <ul className="space-y-3" style={{ fontWeight: 600, fontSize: '13px', color: '#334155' }}>
            ...
            <li className="flex gap-3 items-start">
              ...
              <span><span className="text-slate-500 font-semibold">找不同難度：</span>{spotTimeLimitSec}s / 試錯 {spotMaxMistakes} 次</span>
            </li>
            <li className="flex gap-3 items-start">
              <span className="shrink-0 w-2 h-2 rounded-full mt-1.5 bg-violet-500" />
              <span><span className="text-slate-500 font-semibold">記憶配對難度：</span>展示 {memoryShowSeconds.toFixed(1)}s、作答 {memoryPlayLimitSec}s</span>
            </li>
            <li className="flex gap-3 items-start">
